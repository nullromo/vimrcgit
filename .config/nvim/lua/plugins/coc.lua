    ...
    return {
        ...
        config = function()
            ...
            vim.keymap.set('i', '<C-n>',
                ...
                { expr = true, desc = 'CoC autocomplete next' }
            )
            vim.keymap.set('i', '<C-p>',
                ...
                { expr = true, desc = 'CoC autocomplete previous' }
            )

            -- move between errors
            vim.keymap.set('n', '[[', '<Plug>(coc-diagnostic-prev)',
                { remap = true, silent = true, desc = 'CoC next diagnostic' }
            )
            vim.keymap.set('n', ']]', '<Plug>(coc-diagnostic-next)',
                { remap = true, silent = true, desc = 'CoC previous diagnostic' }
            )
            ...
            vim.api.nvim_create_user_command('Check', ':CocDiagnostics()<CR>',
            ...
            vim.keymap.set('n', 'gy', '<Plug>(coc-type-definition)',
                { remap = true, silent = true, desc = 'CoC jump to type definition' }
            )
            vim.keymap.set('n', 'gi', '<Plug>(coc-implementation)',
                { remap = true, silent = true, desc = 'CoC jump to implementation' }
            )
            ...
            vim.keymap.set('n', '[g', '<Plug>(coc-git-prevchunk)',
                { remap = true, silent = true, desc = 'CoC previous git change' }
            )
            ...
            vim.keymap.set('n', '[c', '<Plug>(coc-git-prevconflict)',
                { remap = true, silent = true, desc = 'CoC previous git conflict' }
            )
            ...
            vim.api.nvim_create_user_command('CocAction', 'normal <Plug>(coc-codeaction)',
            ...
            vim.api.nvim_create_user_command('Rename', 'normal <Plug>(coc-rename)',
            ...
            vim.api.nvim_create_user_command('GHeader', 'CocCommand clangd.switchSourceHeader',
                { bang = true, desc = 'CoC jump to header file' }
            )
            vim.api.nvim_create_user_command('Header', 'rightbelow sp | CocCommand clangd.switchSourceHeader',
                { bang = true, desc = 'CoC open header file horizontally' }
            )
            vim.api.nvim_create_user_command('VHeader', 'rightbelow vsp | CocCommand clangd.switchSourceHeader',
                { bang = true, desc = 'CoC open header file vertically' }
            )

            -- scroll popups with <C-f> and <C-b>
            vim.keymap.set('n', '<C-f>', 'coc#float#scroll(1)',
                { silent = true, nowait = true, expr = true, desc = 'CoC scroll popup down' }
            )
            vim.keymap.set('n', '<C-b>', 'coc#float#scroll(0)',
                { silent = true, nowait = true, expr = true, desc = 'CoC scroll popup up' }
            )
        end,
        lazy = false,
    }

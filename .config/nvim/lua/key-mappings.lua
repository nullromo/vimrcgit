return function()
    ...
    vim.keymap.set('t', '<C-k>', '<C-w>k', { desc = 'move to terminal window above' })
    vim.keymap.set('t', '<C-j>', '<C-w>j', { desc = 'move to terminal window below' })
    vim.keymap.set('t', '<C-h>', '<C-w>h', { desc = 'move to terminal window left' })
    vim.keymap.set('t', '<C-l>', '<C-w>l', { desc = 'move to terminal window right' })
    -- Move around in insert mode faster
    vim.keymap.set('i', '<C-k>', '<C-o>k', { desc = 'move in insert mode up' })
    vim.keymap.set('i', '<C-j>', '<C-o>j', { desc = 'move in insert mode down' })
    vim.keymap.set('i', '<C-h>', '<C-o>h', { desc = 'move in insert mode left' })
    vim.keymap.set('i', '<C-l>', '<C-o>l', { desc = 'move in insert mode right' })
    ...
    vim.keymap.set('t', ';', '<C-w>:', { desc = 'enter ex mode in terminal window' })
    ...
    vim.keymap.set('t', '<C-z>', '<C-w>:suspend<CR>', { desc = 'suspend vim from terminal window' })

    -- Use gt and gT to go to tabs from within terminal windows
    vim.keymap.set('t', 'gt', '<C-w>gt', { desc = 'next tab in terminal window' })
    vim.keymap.set('t', 'gT', '<C-w>gT', { desc = 'previous tab in terminal window' })

    -- Use <C-'> to paste in terminal windows
    vim.keymap.set('t', "<C-'>", '<C-w>""', { desc = 'paste in terminal window' })

    -- Use <C-w><C-w> to enter terminal normal mode
    vim.keymap.set('t', '<C-w><C-w>', '<C-w>N', { desc = 'enter terminal normal mode' })
    ...
    vim.keymap.set('n', '0', 'g0', { desc = 'jump to beginning of line across line wrap' })
    vim.keymap.set('n', '^', 'g^', { desc = 'jump to beginning of text across line wrap' })
    vim.keymap.set('n', '$', 'g$', { desc = 'jump to end of line across line wrap' })

    -- Use space to center the screen
    vim.keymap.set({'n', 'v'}, '<space>', 'zz', { desc = 'center the screen' })

    -- Use , + direction to resize windows
    vim.keymap.set('n', '<Leader>h', ':vertical resize +3<CR>', { desc = 'window size increase horizontally' })
    vim.keymap.set('n', '<Leader>j', ':resize +3<CR>', { desc = 'window size increase vertically' })
    vim.keymap.set('n', '<Leader>k', ':resize -3<CR>', { desc = 'window size decrease vertically' })
    ...
    vim.keymap.set('c', 'W<CR>', "getcmdtype() =~ '^[:]$' ? 'w<CR>' : 'W<CR>'", { expr = true, desc = 'save file with W' })
    vim.keymap.set('c', 'X<CR>', "getcmdtype() =~ '^[:]$' ? 'x<CR>' : 'X<CR>'", { expr = true, desc = 'quit with X' })
    vim.keymap.set('c', 'Q<CR>', "getcmdtype() =~ '^[:]$' ? 'q<CR>' : 'Q<CR>'", { expr = true, desc = 'close file with Q' })
    ...
    vim.api.nvim_create_autocmd({'FileType'}, {pattern = 'javascript,typescript,javascriptreact,typescriptreact', command = 'inoremap <buffer> clog<TAB> console.log();<ESC>hi', desc = 'javascript println clog' })
    vim.api.nvim_create_autocmd({'FileType'}, {pattern = 'javascript,typescript,javascriptreact,typescriptreact', command = 'inoremap <buffer> sout<TAB> console.log();<ESC>hi', desc = 'javascript println sout' })
    ...
    vim.keymap.set('c', 'vt', 'vertical terminal', { desc = 'open vertical terminal' })
